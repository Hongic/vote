...
if (! function_exists ( 'pageft' )) {
	function pageft($totle, $displaypg =10, $shownum = 0, $showtext = 0, $showselect = 0, $showlvtao = 7, $showselects = 20, $showjump = 0, $url = '') {
		...
		if ($url_query) {
			$url_query = @ eregi_replace ( "(^|&)page=$page", "", $url_query );
		...
		if ($page > $lastpg)
			$page = $lastpg;
		if ($showtext == 1) {
			$pagenav = "<span class='disabled'>" . " 共</span><span class='disabled'> $page/$lastpg 页</span>";
		} else {
			$pagenav = "";
		}
		if ($lastpg <= 1)
